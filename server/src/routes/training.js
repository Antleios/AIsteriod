...
router.post('/care-assignments', async (req, res, next) => {
  try {
...
router.post('/sessions/:sessionId/game-runs', async (req, res, next) => {
  try {
...
router.post('/sessions/:sessionId/questions/:questionId/attempts', async (req, res, next) => {
  try {
...
router.post('/sessions/:sessionId/events', async (req, res, next) => {
  try {
    ...
    if (!input) return
    res.status(201).json({ result: await recordInteractionEvents(req.auth.user, req.params.sessionId, input) })
  } catch (error) {
    next(error)
  }
})

router.post('/sessions/:sessionId/conversation-turns', async (req, res, next) => {
  try {
...
router.post('/sessions/:sessionId/finalize', async (req, res, next) => {
  try {
